...
export const actions = {
  ...
  async updatePage({ dispatch, commit, state }, { page, type }) {
    ...
    // Check if filters or search are active (similar to resetState logic)
    const hasActiveFiltersOrSearch =
      state[type].filtersCount > 0 ||
      (state[type].search && state[type].search !== '')

    // Only use database (update) if filters/search active, otherwise rely on SSR payload
